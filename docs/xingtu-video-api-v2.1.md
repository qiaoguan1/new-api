...
| `running`（`recovery.phase=reconciling`） | 中转站正在用同一幂等键和上游账单自动确认；下游只需继续查询 |
| `failed` | 自动恢复已达到有界终点；预扣已按最终 `billing` 字段处理 |
...
| `recovery_pending` | 中转站正在核对失败尝试的真实净成本；下游不参与上游处理 |
...
- 不确定提交由中转站自动核对；下游禁止生成新请求ID，继续查询原任务。
- 429和可重试5xx：指数退避，但仍使用同一请求ID和同一请求内容。
- 请求超时不代表提交失败；中转站只用同一幂等键确认，取得可信退款/成本证据后才自动换路。
