...
- Windows本地 `go test ./...`除根包外均通过；根包仅因本地未生成 `web/classic/dist`
  嵌入资源而无法编译。生产Docker构建已生成前端资源并成功完成全部镜像构建。
- Linux生产构建环境已执行 `go test -race ./service ./model ./middleware -count=1`并通过。
- 本地跨栈十轮验证和部署后生产十轮验证全部通过。
- 生产NewAPI和视频网关均运行合并提交 `480e767d`，健康检查通过，部署后日志无
  `panic`、`fatal`或Python traceback。
- 生产Webhook继续保持关闭，直到下游确认生产/测试回调地址和密钥安全交付方式；该项由
  运维子任务 #60 跟踪，不影响查询接口和当前视频生成。
