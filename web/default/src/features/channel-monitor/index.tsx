...
export function ChannelMonitor() {
  ...
  const runtimeRows = useMemo(
    () =>
      [...(monitor?.upstreams || [])].sort(
        (a, b) => (b.calls_24h || 0) - (a.calls_24h || 0)
      ),
    [monitor]
  )
  ...
  return (
    <SectionPageLayout>
      ...
      <SectionPageLayout.Description>
        运行指标按滚动 24 小时更新；收入、上游真实成本和毛利按上一个完整 UTC
        日核算，金额均为人民币。
      </SectionPageLayout.Description>
      <SectionPageLayout.Actions>
        <Button size='sm' render={<a href='/channels?action=create' />}>
          <ListPlus />
          添加渠道
        </Button>
        <Button variant='outline' size='sm' render={<a href='/channels' />}>
          <Settings />
          管理渠道
        </Button>
        <Button
          variant='outline'
          size='sm'
          render={<a href='/channel-monitor/upstreams-admin.html' />}
        >
          <KeyRound />
          上游采集配置
        </Button>
        ...
        <div className='space-y-6'>
          ...
          <div>
            <div className='mb-3 flex items-end justify-between gap-4'>
              <div>
                ...
                <p className='text-muted-foreground text-sm'>
                  核算日期：{business.date || '等待数据'}
                  ；毛利已包含未归属上游成本。
                </p>
              </div>
              <span className='text-muted-foreground text-xs'>
                {business.generated_at_iso || ''}
              </span>
            </div>
            <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-6'>
              <MetricCard
                icon={WalletCards}
                title='站内消耗金额'
                value={money(bt.revenue_cny, 6)}
                detail={`${number(bt.calls)} 次调用`}
              />
              <MetricCard
                icon={CircleDollarSign}
                title='上游真实成本'
                value={money(bt.upstream_cost_cny, 6)}
                detail={`已归属 ${money(bt.allocated_cost_cny, 6)}`}
              />
              <MetricCard
                icon={TrendingUp}
                title='毛利额'
                value={money(bt.gross_profit_cny, 6)}
                detail='站内收入 - 全部上游成本'
                valueClass={marginClass(bt.gross_margin)}
              />
              <MetricCard
                icon={Activity}
                title='综合毛利率'
                value={percent(bt.gross_margin)}
                detail={`低毛利渠道 ${number(bt.risk_channels)} 个`}
                valueClass={marginClass(bt.gross_margin)}
              />
              <MetricCard
                icon={TriangleAlert}
                title='未归属成本'
                value={money(bt.unallocated_cost_cny, 6)}
                detail='无法匹配渠道/模型的成本'
                valueClass={
              ...
              <MetricCard
                ...
                value={
                  bt.calls
                    ? `${number(((bt.success_calls || 0) / bt.calls) * 100, 2)}%`
                    : '-'
                }
                detail={`${number(bt.error_calls)} 次错误`}
              />
            </div>
          </div>
          ...
          {(bt.unallocated_cost_cny || 0) > 0 && (
            <div className='rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700'>
              当前有 {money(bt.unallocated_cost_cny, 6)} 上游成本无法和 NewAPI
              同日模型日志匹配。总毛利已扣除该成本，但不会强行分摊到具体渠道。
            </div>
          )}
                  ...
                  {businessRows.map((row) => (
                    <TableRow key={row.channel_id}>
                      ...
                      <TableCell className='text-right'>
                        {number(row.calls)}
                        <div className='text-muted-foreground text-xs'>
                          {number(row.success_rate, 2)}%
                        </div>
                      </TableCell>
                      <TableCell className='text-right font-medium'>
                        {money(row.revenue_cny, 6)}
                      </TableCell>
                      <TableCell className='text-right'>
                        {money(row.upstream_cost_cny, 6)}
                      </TableCell>
                      <TableCell
                        className={`text-right font-medium ${marginClass(row.gross_margin)}`}
                      >
                        {money(row.gross_profit_cny, 6)}
                      </TableCell>
                      <TableCell
                        className={`text-right font-semibold ${marginClass(row.gross_margin)}`}
                      >
                        {percent(row.gross_margin)}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant={
                            row.cost_source === 'upstream_log'
                              ? 'default'
                              : 'secondary'
                          }
                        >
                        ...
                        <div className='text-muted-foreground mt-1 text-xs'>
                          置信度 {percent(row.cost_confidence)}
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>归属状态</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {modelCosts.map((row, index) => (
                    <TableRow
                      key={`${row.upstream_slug}-${row.model}-${index}`}
                    >
                      ...
                      <TableCell>
                        {row.kind === 'image' ? '按次/图片/视频' : '文字 Token'}
                      </TableCell>
                      <TableCell className='text-right'>
                        {number(row.calls)}
                      </TableCell>
                      <TableCell className='text-right'>
                        {row.cost_cny_per_image !== null &&
                        row.cost_cny_per_image !== undefined
                          ? `${money(row.cost_cny_per_image, 6)}/次`
                          : `${money(row.input_cost_cny_per_m, 6)}/M 输入`}
                      </TableCell>
                      <TableCell className='text-right font-medium'>
                        {money(row.total_cost_cny, 6)}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant={row.allocated ? 'default' : 'destructive'}
                        >
                          {row.allocated ? '已归属渠道' : '未归属'}
                        </Badge>
                      </TableCell>
                    </TableRow>
                  ))}
          ...
          <div>
            ...
            <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-5'>
              <MetricCard
                icon={Activity}
                title='24h 成功率'
                value={`${number(successRate, 2)}%`}
                detail={`${number(totals.calls_24h)} 次调用 / ${number(totals.errors_24h)} 次错误`}
              />
              <MetricCard
                icon={WalletCards}
                title='24h 站内消耗'
                value={money(totals.cost_24h_usd, 6)}
                detail={`7 天 ${money(totals.cost_7d_usd, 6)}`}
              />
              <MetricCard
                icon={Server}
                title='渠道状态'
                value={`${number(totals.enabled_channels)} / ${number(totals.channels)}`}
                detail={`${number(totals.upstreams)} 个上游站点`}
              />
              <MetricCard
                icon={TriangleAlert}
                title='当前告警'
                value={number(totals.alerts)}
                detail='余额、错误率和响应速度'
              />
              <MetricCard
                icon={RefreshCw}
                title='更新时间'
                value={
                  monitor?.generated_at_iso
                    ? new Date(monitor.generated_at_iso).toLocaleTimeString(
                        'zh-CN'
                      )
                    : '-'
                }
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className='text-right'>站内消耗</TableHead>
                    <TableHead>最近错误</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {runtimeRows.map((row) => {
                    const rate = row.calls_24h
                      ? ((row.success_24h || 0) / row.calls_24h) * 100
                      : 0
                    return (
                      <TableRow key={row.slug}>
                        <TableCell>
                          ...
                          <div className='text-muted-foreground text-xs'>
                            {number(row.enabled_channels)} /{' '}
                            {number(row.channel_count)} 通道启用
                          </div>
                        </TableCell>
                        <TableCell>
                          <Badge variant={healthVariant(row.health)}>
                            {row.health_label || row.health || '未知'}
                          </Badge>
                        </TableCell>
                        <TableCell className='text-right'>
                          {number(rate, 2)}%
                        ...
                        <TableCell className='text-right'>
                          {number(row.calls_24h)}
                        </TableCell>
                        <TableCell className='text-right'>
                          {row.avg_response_ms
                            ? `${number(row.avg_response_ms)} ms`
                            : '-'}
                        </TableCell>
                        <TableCell className='text-right'>
                          {money(row.cost_24h_usd, 6)}
                        </TableCell>
                        <TableCell
