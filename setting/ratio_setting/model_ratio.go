...
func GetCompletionRatio(name string) float64 {
	...
	// An explicit option value is authoritative for every model. Family defaults
	// remain fallbacks, so newly discovered models can be repriced without
	// requiring another source-code exception.
	...
	if strings.HasPrefix(name, "gpt-") {
		...
		if strings.HasPrefix(name, "gpt-5") {
			if strings.HasPrefix(name, "gpt-5.5") {
				// Allow the database CompletionRatio to override the default.
				return 6, false
			}
