...
func fixture(t *testing.T) (*server, *gin.Engine, model.User, model.Token) {
	...
	require.NoError(t, db.AutoMigrate(&model.User{}, &model.Token{}, &model.Log{}, &model.PublicVideoTask{}, &model.PublicVideoCacheEvent{}))
	mr := miniredis.RunT(t)
...
func TestPublicVideoHTTPIsolationAndRestartSettlement(t *testing.T) {
	...
	require.NoError(t, model.DB.First(&row, "id = ?", id).Error)
	restarted := &server{backend: backend.URL, client: s.client, serviceToken: s.serviceToken, publicURL: s.publicURL, rate: s.rate}
...
func TestPublicVideoCacheDeliveryIsIdempotent(t *testing.T) {
	_, r, u, token := fixture(t)
	...
	require.NoError(t, model.DB.Model(&model.PublicVideoCacheEvent{}).Where("delivered = ?", true).Update("delivered", false).Error)
	require.NoError(t, model.FlushPublicVideoCache(""))
	q, e := common.RDB.HGet(ctx, uk, "Quota").Int()
	require.NoError(t, e)
