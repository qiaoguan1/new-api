...
def validate(raw,provider):
    ...
    if not isinstance(size,str) or size not in SIZES:raise Rejection('invalid_size','Use 1024x1024, 2048x2048 or 4096x4096')
    ...
    if provider=='rolldek' and size!='1024x1024':raise Rejection('unsupported_tier','This route supports 1K only; use the other verified route',429)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            self.send(200,{'object':'list','data':[{'id':MODEL,'object':'model','owned_by':'upstream-relay'}]})
    ...
    def do_POST(self):
        ...
        try:
            ...
            self.send(e.status,{'error':{'code':e.code,'message':e.message,'type':'image25_adapter_error'}})
