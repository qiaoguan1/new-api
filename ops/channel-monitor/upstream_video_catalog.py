        ...
        for source, account in target_accounts.items():
            ...
            for row in metadata.get("models") or []:
                if not isinstance(row, dict):
                    continue
                billing_mode = _text(row.get("billing_mode")).casefold()
                quota_type = row.get("quota_type")
                quota_type_is_fixed = quota_type == 1 and not isinstance(quota_type, bool)
                if billing_mode != "per_call" and not quota_type_is_fixed:
                    continue
