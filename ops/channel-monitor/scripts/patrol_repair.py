...
def pricing_business_health(run: Mapping[str, Any]) -> tuple[str, str]:
    """Distinguish an executed worker from a successfully reconciled price run."""
    if run.get("error") or run.get("status") == "failed":
        return "failed", "scheduled_run_failed"
    ...
    if status in {"partial", "blocked"}:
        return "warning", "pricing_business_" + status
...
class PatrolChecks:
    ...
    def _artifact(self, item: Mapping[str, Any], now: int) -> CheckResult:
        ...
        if artifact_type == "audit":
            ...
            failed_channels = summary.get("failed_channels") if isinstance(summary, dict) else None
            if isinstance(failed_channels, bool) or not isinstance(failed_channels, int) or failed_channels < 0:
                failed_channels = sum(
                    1
                    for row in document.get("channels") or []
                    if isinstance(row, dict)
                    and str(row.get("status")) == "1"
                    and row.get("scan_status") != "ok"
                )
            if healthy and failed_channels > 0:
