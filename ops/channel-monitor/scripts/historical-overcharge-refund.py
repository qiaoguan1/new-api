...
def build_evidence_index(ledger, excluded_sources=EXCLUDED_SOURCES):
    """Select conservative worst actual cost for every Beijing day/model."""
    ...
    for day, sources in sorted(days.items()):
        ...
        included_entries = [
            ...
            if _normal_model(source) not in excluded_sources
        ]
        modern_entries = [
            entry
            for entry in included_entries
            if isinstance(entry, dict)
            and ("collection_status" in entry or "actual_log_complete" in entry)
        ]
        if modern_entries and any(
            _source_quality(entry) != "complete_actual_log" for entry in included_entries
        ):
            continue
        for source, entry in sorted(sources.items()):
            if _normal_model(source) in excluded_sources:
                continue
            ...
            for display_model, cost in sorted(costs.items()):
                ...
                try:
                    ...
                    continue
                candidates[(str(day), _normal_model(display_model), item["kind"])].append(item)

    index = {}
    keys_by_model = defaultdict(set)
    for day, model, kind in candidates:
        keys_by_model[(day, model)].add(kind)
    for key, kinds in sorted(keys_by_model.items()):
        if len(kinds) != 1:
            continue
        kind = next(iter(kinds))
        rows = candidates[(key[0], key[1], kind)]
        complete = [row for row in rows if row["quality"] == "complete_actual_log"]
        eligible = complete or rows
        quality = "complete_actual_log" if complete else "legacy_actual_sample"
        evidence = {
            "day": key[0],
            "model": eligible[0]["model"],
            "kind": kind,
            "quality": quality,
            "observed_sources": sorted({row["source"] for row in eligible}),
            "observed_calls": sum(row["calls"] for row in eligible),
        }
        if kind == "text":
            input_row = max(eligible, key=lambda row: row["input_cost_cny_per_m"])
            output_row = max(eligible, key=lambda row: row["output_cost_cny_per_m"])
            evidence.update(
                input_cost_cny_per_m=input_row["input_cost_cny_per_m"],
                input_source=input_row["source"],
                output_cost_cny_per_m=output_row["output_cost_cny_per_m"],
                output_source=output_row["source"],
            )
        else:
            fixed_row = max(eligible, key=lambda row: row["cost_cny_per_call"])
            evidence.update(
                cost_cny_per_call=fixed_row["cost_cny_per_call"],
                fixed_source=fixed_row["source"],
            )
        index[key] = evidence
    return index
...
def build_refund_plan(logs, evidence_index, already_refunded=None):
    ...
    for log in sorted(logs or [], key=lambda item: int(item.get("id") or 0)):
        ...
        evidence = evidence_index.get((day, model))
        if not evidence:
...
def create_backup(plan, target):
    ...
    backup = {
        ...
        "source_logs": _json_lines(
            f"SELECT json_build_object('id',id,'user_id',user_id,'token_id',token_id,'created_at',created_at,'type',type,'model_name',model_name,'quota',quota,'prompt_tokens',prompt_tokens,'completion_tokens',completion_tokens,'request_id',request_id,'other_md5',md5(other))::text FROM logs WHERE id IN ({log_list}) ORDER BY id;"
        ),
    }
...
def _execution_sql(plan, *, commit=True):
    ...
    return f"""
BEGIN;
SELECT pg_advisory_xact_lock(hashtext('issue-21-historical-overcharge-refunds'));
...
WITH inserted AS (
  INSERT INTO historical_pricing_refunds (
    source_log_id,issue_number,plan_sha256,user_id,token_id,model_name,beijing_date,
    original_quota,policy_quota,refund_quota,evidence,source_fingerprint,created_at
  ) SELECT source_log_id,{ISSUE_NUMBER},{_sql(plan_sha)},user_id,token_id,model_name,
...
) SELECT a.user_id,{now},6,'Issue #21 historical overcharge compensation',u.username,'','',
  a.refund,0,0,0,false,0,0,'','','issue21:' || left({_sql(plan_sha)},32) || ':' || a.user_id,
  json_build_object('issue',21,'plan_sha256',{_sql(plan_sha)},'source_log_count',a.requests,
...
def main(argv=None):
    ...
    if not args.apply and not args.validate_transaction:
        ...
        plan = build_refund_plan(logs, build_evidence_index(ledger), existing)
    ...
    backup_path = args.backup_dir / f"issue21-{timestamp}-{plan_sha[:12]}.json"
